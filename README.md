...
flowchart TB
    ...
    subgraph Backend["🪝 AgentTether Backend — Node.js / Express / TS"]
        ...
        Engine["settlementEngine.ts<br/>block-metered settle · dual-guard cron (block budget | TTL)"]
    ...
    EP -->|"gRPC blocks · head streaming"| Stream
    SPKG -.->|"static input"| EP
    Stream -->|"match condition → delivery queue · meter blocks (block arithmetic)"| Engine
    API -->|"verify (once, deferred settle)"| FAC
    Engine -->|"/settle (pro-rata blocks, at termination)"| FAC
    FAC --> BASE
...
### End-to-end block-metered flow (demo beat A)
...
sequenceDiagram
    ...
    API-->>A: 402 PAYMENT-REQUIRED (upto · quoted ceiling = budget blocks × rate · facilitatorAddress)
    ...
    S->>API: block stream (in-process · head streaming)
    Note over API: condition matched → delivery queue<br/>blocks accounted by block-number arithmetic at termination
    API->>F: /settle (actual = blocks_consumed × rate ≤ quoted ceiling)
...
### Intent lifecycle (stream endpoint)
...
stateDiagram-v2
    ...
    MONITORING --> SETTLED: condition fired → /settle(blocks processed)
    MONITORING --> TIMEOUT: window over (block budget | TTL) → settle blocks processed
...
- [x] **1.3** Define the `schema.prisma` file with an `Intent` model (renamed from `ChronosIntent` — stale branding). Include fields:
  ...
  - `per_block_rate_atomic` (String — USDC atomic units; the billing unit is a **processed block**, not an event) · `budget_blocks` (Int — the quoted window: `⌈ttl ÷ blockTime⌉`) · `start_block_num` (Int, nullable — set lazily at the first in-window block the stream processes; billing is `cursor.blockNum − startBlockNum + 1`, computed, never a counter)
...
- [x] **3.4** Implement `POST /api/v1/intents/oneshot` — a flat-fee, non-metered pull endpoint, with **per-network facilitator routing**. **Done + live-verified** (`spikes/oneshot-client.ts`): unpaid POST → `402` quoting the flat fee; paid POST → `200` with the lookback results **and a confirmed settle receipt** (auto-settled by the standard `paymentMiddleware` after the handler responds — the flow `/stream` deliberately bypasses); a second paid call settled again on a fresh voucher (nonce rotation). One run also caught a transient facilitator verify bounce (silent `402 {}`) that passed on retry — same flakiness family as the settle bounces in 4.2.
  - **Capture (the data source):** the dataplane writes every allowlisted transfer into `ProcessedTransfer` **in the same per-block transaction as metering + the cursor** — the cursor never advances past uncaptured blocks. Allowlist = `ONESHOT_CONTRACTS` (default mainnet USDC). Rows carry the canonical `0x`-prefixed web3 form (unlike the bare-hex webhook payload convention — the exact bug the first live run caught). Two prunes keep it bounded: retention (`ONESHOT_RETENTION_HOURS`, default 2) and post-undo pruning of anything above the reverted cursor.
  - **Rails are equal citizens:** `accepts` is an array of `PaymentOption`s — one per network, each with its own `payTo`/price; the per-network facilitator routing is the facilitator-client array (one `HTTPFacilitatorClient` per rail). Base Sepolia `exact` USDC is live at `ONESHOT_PRICE_BASE_ATOMIC` (default 500 atomic = $0.0005/lookup, server-owned env pricing like everything else).
  - **Lookback:** the handler queries `[head − lookback_blocks, head]` behind the stream cursor, clamped server-side (`ONESHOT_LOOKBACK_BLOCKS_MAX`, default 300; result limit ≤ 500); `min_amount_atomic` filters server-side on an exact `BigInt` column. `lookback_blocks` beyond the max is clamped, not rejected — the capture window is the honest bound.
  - **Hedera rail — DONE + live-verified:** `hedera:testnet` `exact` HBAR (asset `0.0.0`, 8-decimal tinybars, `ONESHOT_PRICE_HEDERA_TINYBAR` default 10_000 = 0.001 ℏ/lookup). **The routing order IS the routing:** Blocky402 first in the facilitator-client array makes it the hedera primary (its `GET /supported` supplies `extra.feePayer` — observed settle tx `0.0.7162784@…`); the default x402.org facilitator stays second as the degraded-mode fallback (risk #4) and still owns Base (Blocky402 doesn't advertise `eip155:84532`). `HEDERA_PAY_TO` accepts a `0x…` EVM address or a `0.0.x` id — the `0x` form is resolved to its entity id via the testnet mirror at mount (`0x91Accc…B342` → `0.0.10390121`); the rail stays unadvertised while the account doesn't exist yet. Two Hedera gotchas the live beat surfaced: a payer account created by an incoming HBAR transfer is *hollow* (`key: null`) until its ECDSA key signs once (`spikes/hedera-activate.ts` sends 1 tinybar — Blocky402's verify fetches the on-chain key and would fail closed on null), and the x402 client's spend controls reject HBAR as a non-default asset (the demo client disables them — real clients would whitelist the asset).
  ...
  - **Dynamic facilitator override (bounty-driven, not technical):** maintain a network→facilitator routing map. `eip155:*` payloads go to the default `FACILITATOR_URL`; any payload whose `accepted.network` matches `hedera:*` has its `/verify` and `/settle` calls dynamically rerouted to `HEDERA_FACILITATOR_URL` (Blocky402, as named by the Hedera bounty). Important correction: the default x402.org facilitator **does** natively support `exact` on `hedera:testnet` (verified via `/supported`: `signers["hedera:*"] = ["0.0.9185802"]`) — the routing exists because the bounty mandates Blocky402, not because Hedera is otherwise unsettleable. Implemented as the facilitator-client array with Blocky402 first for hedera; a routing wrapper remains the fallback if Blocky402 needs request-specific behaviour.
  - The Hedera `PaymentRequirements` include `extra.feePayer`, discovered from Blocky402's `GET /supported` (`signers["hedera:*"][0]` = `0.0.7162784`) — Blocky402 co-signs the `TransferTransaction` as fee-payer at settlement time, and verification rejects a mismatched fee-payer. Never hardcode it.
  - **Demo beat (done):** paid 0.001 ℏ on Hedera → `200` + settle receipt (payer `0.0.10383384`, fee payer `0.0.7162784`) → HashScan receipt: https://hashscan.io/testnet/transaction/0.0.7162784-1788684850-502280369

### Phase 4: Escrow & Settlement Engine
> **Goal:** Manage the financial lifecycle of the vouchers (pro-rata block settlement or window expiry).
...
- [x] **4.2** Create the `executeSuccessSettlement(intentId)` function. **Live-verified (per-block model):** fresh real voucher → condition matched in the first in-window block → `/settle` for exactly 1 block × rate (pro-rata; idle window settles its full budget) → receipt-gated webhook delivered the matched events + `{tx_hash, amount_charged_atomic}`. *(Pre-pivot record: the per-event settle path was verified the same way — 28 matches → metered actual → receiver at 0.206238 USDC across several real partial settles.)*
  ...
  - **Settlement idempotence (implementation recipe):** the engine must be safe if invoked twice — Phase 4 has two trigger paths (event-fired via 2.6 and the TTL cron) plus crash-recovery re-fires (see 4.3's startup sweep), so `executeSuccessSettlement` may race itself. Three layers, each covering the previous one's failure mode:
    ...
    Worst case across any crash/retry sequence: **settled exactly once** or **not yet settled (recoverable)** — never "settled twice" or "settled but the DB doesn't know." Footnote: the status guard presumes a **single writer** — one backend instance streaming against the DB (two instances would double-*meter*, though still not double-*charge*). **Enforced in code:** the stream claims a heartbeat lease (`process_lease`) before starting; a second instance exits non-zero, and its supervisor retries until the dead holder's lease goes stale (graceful shutdown releases instantly) — see `deploy/README.md` for the daemon runbook. **Bonus finding (live):** the Permit2 *allowance-mode* approval **decrements per settlement** — the "approve max once" bootstrap is not permanently sufficient; the client-side pre-check must compare `allowance ≥ ceiling` on every run (the stream client already does, so re-bootstrap is automatic when erosion crosses the next ceiling).
...
- [x] **5.3** LangGraph node `negotiate` (renamed from `request_data_stream`).
  - The wrapped session (`x402_session.py`) sends the POST; the Python SDK parses the 402's `PaymentRequired` and signs — agent code never hand-signs on the happy path.
  - An `on_payment_required` hook records the quoted ceiling (the client's worst case) for the narration.
- [x] **5.4** The SDK covers `sign_and_retry` inside the wrapped session (structure + EIP-712 signature + `PAYMENT-SIGNATURE` retry, exactly as specified) — the agent's own code stays out of the signing path (the LLM never touches keys; its worst case is the quoted ceiling).
- [x] **5.5** Flask receiver (`webhook_server.py`) — buffers notices per intent AND resumes the graph: the backend's webhook (delivery or timeout) hits `POST /hook`, which drives `Command(resume=notice)` into the parked thread. `POST /resume/:intent_id` is the manual/ops trigger. **The pause is real:** `wait_for_webhook` raises `interrupt()`; the invoke returns and nothing burns until the webhook lands — visible in LangGraph Studio (`agent/langgraph.json` → `studio.py:graph`), where the graph visibly parks in Monitoring state.
- [x] **5.5a** **Transparency in the agent's prompts:** the disclosure is a constant in `graph.py`, appended to every narration (LLM-enriched or the factual baseline), so it appears in logs and transcripts as a matter of course. include an explicit disclosure in the agent's system prompt, e.g. *"You are observing Ethereum/Base mainnet for data velocity (real, organic on-chain events), but all x402 financial settlement executes safely on Base Sepolia testnet. The observed data and the payment rail are on different chains by design."* The agent should also state this split in its final summary output when reporting results, so the acknowledgment appears in logs and transcripts as a matter of course.
- [x] **5.6** The demo scripts, all live-verified:
  - **(A)** `scripts/demo_agent.py` — the agentic run: plan → negotiate (quoted ceiling surfaced) → **pause** → webhook (51 events, 100 atomic pro-rata) → the decide node **re-issued** another window → parked again → woke → done, with the disclosure. The continuity path of the per-block model, driven by the agent itself.
  - **(B)** `scripts/spike_e2e.py` beat B — idle window → `intent.timeout` → full block-budget settlement (500 atomic; explorer link printed).
  - **(C)** Hedera one-shot — `spikes/oneshot-client.ts` beat 4 (the Python SDK has no Hedera scheme; the Node script builds the partially-signed `TransferTransaction` Blocky402 co-signs).
  - The LLM layer is `opencode` by default (the running `opencode serve` carries the user's auth — no extra API key) with a deterministic `scripted` fallback that runs every beat keyless (CI uses it).
  - **Shareable rendering + flow-chart:** every watch gets a URL — the agent's output links `…/w/<intent-id>`, a page that renders the request **as a vertical stepper with ✓ per step and start timestamps** (requested → paid → window opened → settled/expired → webhook delivered, plus `agent:` annotation rows the LLM nodes append via the length-capped `POST /api/v1/intents/:id/annotate`), above the events table (mainnet explorer links) and the settlement card (Base Sepolia link). The lifecycle is persisted on the intent row (`lifecycle` JSON, appended at each transition). The home page lists the `RECENT_INTENTS_LIMIT` (default 5) most recent requests — **wallet-free** — each linking to its chart; the data is public but unguessable (UUID ids). Report links handed to agents ride the 202 response and webhook notices as `report_url` (base: request host, or `PUBLIC_BASE_URL` for tunnels). Served by the backend; the same pages deploy to Netlify (`web/` + `netlify.toml`, `?api=` for a remote backend). LangGraph Studio verified through the platform API: the thread parks on `wait_for_webhook` (`interrupts: ['monitoring']`) and a `Command(resume)` completes — see `agent/README.md`.
...
| `/settle` with `amount = 0` | success, **no** on-chain transaction | ✅ PASS (the $0-settle capability; today's timeout settles actual blocks) |
...
1. **`npm test` — fast suite (vitest, ~0.5s, no DB / chain / money):** pure units + mocked HTTP/orchestration, colocated as `*.test.ts`. Covers the matcher's window semantics (catch-up guards, TTL guard, malformed conditions), the engine's rejection triage (every fixture is a reason observed live against the hosted facilitator), pro-rata block amount math, webhook truncation flags, the router's full 3.2/3.3 branch table via a real Express listener with `db.js`/`facilitator.js` mocked, and the engine's complete state machine (success, idle full-budget timeout, degenerate $0, deadline-void, structural, transient, uncertain, CAS no-op, receipt gate, webhook retries). Runs in CI on every push.

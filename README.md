...
> **What the agent pays for:** conditional data delivery — blockchain events matching its declared condition, settled and pushed to its webhook after the condition first fires (the shipped model is **first-match batch settlement**: the first matching block triggers one settlement and delivery of that block's events, metered `events_matched × rate_per_event_atomic`, capped by the ceiling the agent signed; incremental per-event delivery for the rest of the TTL is the registered next step — see risk #8). Idle monitoring is free: an intent that never matches settles **$0** (no on-chain transaction — the authorization simply expires). There is no capital lockup: the voucher *authorizes*, it does not escrow, so funds stay in the agent's wallet until the deferred settlement actually executes. And the agent buys results, not infrastructure — AgentTether absorbs the Substreams endpoint, cursor management, and reorg handling. The one trust assumption (inherited from `upto` itself): the metered count is server-computed, with the signed ceiling bounding the worst case. The one-shot endpoint is the contrasting product: flat fee, immediate response, no TTL, no metering.

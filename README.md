...
- [ ] **5.6** Write a test script running the full end-to-end flow to record for the hackathon demo, covering three outcomes:
  - **(A)** Condition fires → pro-rata settlement for the blocks processed (show Base Sepolia explorer link).
  - **(B)** Window expires (never fired) → full block-budget settlement — the watch was subsidised (explorer link).

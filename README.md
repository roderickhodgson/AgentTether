...
- [x] **5.6** The demo scripts, all live-verified:
  ...
  - **Shareable rendering:** every watch gets a URL — the agent's output links `…/w/<intent-id>`, a human-readable page (events with mainnet explorer links, the settlement receipt on Base Sepolia, the disclosure), backed by a CORS-open `GET /api/v1/intents/:id/report`. Served by the backend; the same page deploys to Netlify (`web/` + `netlify.toml`) and fetches any reachable API via `?api=`. LangGraph Studio verified through the platform API: the thread parks on `wait_for_webhook` (`interrupts: ['monitoring']`) and a `Command(resume)` completes — see `agent/README.md`.
